...
describe("POST /users", function () {
  ...
  test("unauth for normal logged in user", async function () {
    const resp = await request(app)
        .post("/users")
        .send({
          username: "u-new",
...
describe("GET /users/:username", function () {
  ...
  test("unauth for a different user or non-admin", async function () {
    const resp = await request(app)
        .get(`/users/u2`)
        .set('authorization', `Bearer ${u1Token}`);
    expect(resp.statusCode).toEqual(401);
...
describe("PATCH /users/:username", () => {
  ...
  test("unauth for different user or non-admin", async function () {
    const resp = await request(app)
        .patch(`/users/u2`)
        .send({
          firstName: "New",
